...
public class ApplicationMaster {
    ...
    public boolean init(String[] args) throws ParseException, IOException {
        ...
        if (!fileExist(shellCommandPath)
                && envs.get(Constants.DISTRIBUTEDSHELLSCRIPTLOCATION).isEmpty()) {
            throw new IllegalArgumentException(
        ...
        if (envs.containsKey(Constants.DISTRIBUTEDSHELLSCRIPTLOCATION)) {
            scriptPath = envs.get(Constants.DISTRIBUTEDSHELLSCRIPTLOCATION);

            if (envs.containsKey(Constants.DISTRIBUTEDSHELLSCRIPTTIMESTAMP)) {
                shellScriptPathTimestamp = Long.parseLong(envs
                        .get(Constants.DISTRIBUTEDSHELLSCRIPTTIMESTAMP));
            }
            if (envs.containsKey(Constants.DISTRIBUTEDSHELLSCRIPTLEN)) {
                shellScriptPathLen = Long.parseLong(envs
                        .get(Constants.DISTRIBUTEDSHELLSCRIPTLEN));
            }
        ...
        if (envs.containsKey(Constants.DISTRIBUTEDSHELLTIMELINEDOMAIN)) {
            domainId = envs.get(Constants.DISTRIBUTEDSHELLTIMELINEDOMAIN);
        }
